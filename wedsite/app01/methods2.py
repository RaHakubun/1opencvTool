...
def sobel(path):
    ...
    for i in range(0,height-2):
        for j in range(0,width-2):
            result[i+1,j+1]=np.abs(np.sum(img[i:i+3,j:j+3]*s_x))+np.abs(np.sum(img[i:i+3,j:j+3]*s_y))
...
def prewitt(path):
    ...
    for i in range(1,height-2):
        for j in range(1,width-2):
            f_y = np.sum(img[i:i + 3, j:j + 3][2]) - np.sum(img[i:i + 3, j:j + 3][0])
            f_x = np.sum(img[i:i + 3, j:j + 3][:, 2]) - np.sum(img[i:i + 3, j:j + 3][:, 0])
...
def gauss_low_filter(path):
    ...
    for i in range(height):
        for j in range(width):
            d = np.sqrt((i-u)**2 + (j-v)**2)
            img_trans[i][j]=np.exp(-1*(d**2)/(2*(d0**2)))
    #iimg_trans=src_c*img_trans
    iimg_trans=np.multiply(src_shift,img_trans)
    iimg_shift=np.fft.ifftshift(iimg_trans)
    iimg=np.fft.ifft2(iimg_shift)
...
def cny(path):
    ...
    # 3. 求x，y方向的Sobel算子
    gradx = cv2.Sobel(src, cv2.CV_16SC1, 1, 0)  
    grady = cv2.Sobel(src, cv2.CV_16SC1, 0, 1)  
...
def noise_desc(path):
    ...
    result=cv2.cvtColor(output,cv2.COLOR_GRAY2RGB)
    print(np.sum(result))
    return result
